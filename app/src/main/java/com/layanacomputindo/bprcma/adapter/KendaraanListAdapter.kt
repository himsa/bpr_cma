...
import com.layanacomputindo.bprcma.form.KendaraanActivity
import com.layanacomputindo.bprcma.model.Kendaraan
import com.layanacomputindo.bprcma.util.Config

class KendaraanListAdapter(private var mList: ArrayList<Kendaraan>, val context: Context?): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        vh = if (viewType == 1) {
            val v = LayoutInflater.from(parent.context).inflate(R.layout.list_item_cjaminan_kendaraan, parent, false)
            ViewHolder(v)
        } else {
...
//    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ViewHolder){
            holder.tvName.text = mList[position].getNamaPemilik()
            holder.tvHat.text = mList[position].getMerkKendaraan()
            holder.btnDetail.setOnClickListener {
                ...
                editor.apply()
                context.startActivity(Intent(context, KendaraanActivity::class.java))
            }
        }else{
